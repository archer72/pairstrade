...
class PolicyGradient(RNN):
    def buildNetwork(self):
        startlayer = lasagne.layers.LSTMLayer(
            self.Input, 20,
            grad_clipping = 10
            )

        self.network = lasagne.layers.DenseLayer(
            startlayer, num_units=3, nonlinearity=lasagne.nonlinearities.softmax)
...
class ActorCriticAgent(object):
    def __init__(self, n_batch=5, batch_size=64, update_cycle=3, update_target_cycle= 3, lookback=[]):
        ...
        self.choices = []
        self.rewards = []
        self.model = PolicyGradient(self.lookback_size, len(lookback[0]))
